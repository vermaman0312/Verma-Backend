...
export const sendMessage = async (req, res) => {
    try {
        ...
        if (!communication) {
            ...
            return res.json({ Message: "Message Sent", content: newCommunication.communicationContent[0] });
        }
        const newObject = {
            senderId: userId,
            contentPath: null,
        ...
        if (sendedData) {
            ...
            const updateMessage = await Communication.findByIdAndUpdate(communication._id, {
                communicationLastDetails: {
                    lastMessage: content,
                    lastMessageDate: new Date(),
                },
            ...
            if (updateMessage) {
                ...
                return res.json({ Message: "Message Sent", content: lastMessage });
            }
        }
    } catch (error) {
...
export const getParticipant = async (req, res) => {
    try {
        ...
        const newChannelObject = communication.map((response) => ({
            ...
            receiptantDetails: response.communicationMembers.find((id) => id._id.toString() !== userId.toString()),
        }));
...
export const getMessages = async (req, res) => {
    try {
        ...
        const messageObject = content.communicationContent.map((content) => ({
            ...
            direction: content.senderId.toString() === userId.toString() ? "right" : "left",
        }));
